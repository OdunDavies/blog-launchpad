...
import { decodeWorkout, getSharedWorkout, copyToClipboard } from '@/utils/shareWorkout'
import type { DecodedWorkout } from '@/utils/shareWorkout'
...
function SharedWorkoutInner() {
  ...
  const savePlan = () => {
    ...
    try {
      ...
      try { existingPlans = localStorage.getItem(STORAGE_KEY) } catch {}
  ...
  const downloadPlan = async () => {
    ...
    try {
      const { generateWorkoutPdf } = await import('@/utils/downloadHtml')
        ...
        <Card className="max-w-4xl mx-auto">
          <CardHeader>
            <div className="space-y-4">
              <CardTitle className="text-2xl md:text-3xl text-center">{workout.name}</CardTitle>
          ...
          <CardContent className="space-y-6">
              ...
              {workout.schedule.map((day, dayIndex) => (
                <Card key={dayIndex} className="bg-muted/50">
                  <CardHeader className="pb-2">
                    <Badge variant="outline" className="w-fit mb-1">{day.day}</Badge>
                    ...
                    <ul className="space-y-2">
                      {day.exercises.map((exercise, exerciseIndex) => (
                        <li key={exerciseIndex} className="text-sm">
                          ...
                          <p className="text-xs text-muted-foreground">{exercise.sets} \u00d7 {exercise.reps} \u2022 Rest: {exercise.rest}</p>
                        </li>
                      ))}
                    </ul>
                  </CardContent>
            ...
            <div className="text-center pt-4">
              <p className="text-sm text-muted-foreground mb-3">Want to create your own personalized workout?</p>
